...
and bind env pass ((location, expr) : expression) : env * expression =
  ...
  | Lambda (return_type, params, modifiers, statement, _) ->
    ...
    let _, statement = bind_statement (new_scope frame_env) OrderDependent statement in
    env, (location, Lambda (return_type, params, modifiers, (location, BoundFrame (!(frame_env.num_slots), statement)), None))
  ...
  | DynamicArrayType t ->
    let _, t = bind env pass t in
    env, (location, DynamicArrayType t)

  | DynamicArrayLiteral es ->
    let es = Array.map (fun e -> let _, e = bind env pass e in e) es in
    env, (location, DynamicArrayLiteral es)
